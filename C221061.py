...
if uploaded_file is not None:
    ...
    # 프로젝트 주요 특징 섹션
    st.subheader("🚀 프로젝트 주요 특징")
    
    # 레이아웃을 깔끔하게 만들기 위해 2x2 컬럼 또는 카드 형태로 배치 가능합니다.
    col1, col2 = st.columns(2)
    
    with col1:
        st.markdown("""
        ### 📊 1. 실제 데이터 활용
        * **카네이션 판매량 데이터**를 기반으로 현실적이고 실용적인 시계열 분석 환경을 제공합니다.
        
        ### 📈 2. 직관적인 이상치 처리
        * 복잡한 코드 없이 **그래프 상에서 간편하게** 이상치를 탐지하고 손쉽게 처리할 수 있습니다.
        """)
        
    with col2:
        st.markdown("""
        ### 🤖 3. 시계열 예측 모델링 및 검증
        * **[sktime]** 단일 변수 예측 (단변량 모델링)
        * **[Darts]** 다변량 집중 예측 및 과거 데이터 기반 백테스팅 기능 지원
        
        ### 🛠️ 4. 데이터 전처리 및 분석
        * **결측치 선형 보간 처리**를 통한 데이터 무결성 확보
        * **ADF(Augmented Dickey-Fuller) 검정 시각화**를 통한 정상성 분석
        """)
